...
import { fetchRevisionDiff, selectEntry } from '../../store/historySlice'
...
export function DiffPanel({ tab }: Props): JSX.Element {
  ...
  if (tab === 'changes') {
    ...
    return (
      <div className="diff-panel">
        <DiffHeader path={changesDiff.filePath} />
        <DiffContent diff={changesDiff} />
      </div>
    )
  }
  ...
  return (
    <div className="diff-panel">
      ...
      {historyDiff && (
        <>
          <DiffHeader path={historyDiff.filePath} />
          <DiffContent diff={historyDiff} />
        </>
      )}
    </div>
  )
...
function DiffHeader({ path }: { path: string }): JSX.Element {
  return (
    <div className="diff-file-header">
      <span className="diff-file-path">{path.replace(/\//g, '\\')}</span>
      <div className="diff-header-actions">
        <button className="diff-header-btn" title="Diff options">
          <GearIcon />
        </button>
      </div>
    </div>
...
function DiffContent({ diff }: { diff: NonNullable<ReturnType<typeof useSelector<RootState, RootState['changes']['activeDiff']>>> }): JSX.Element {
